...
def init_db():
    ...
    try:
        ...
        print(">>> The API will return errors for database operations.")
        db = None
...
@app.route("/health")
def health():
    return jsonify({"status": "ok", "db": db is not None and db.is_connected()})
...
def get_suggestion(level):
    suggestions = {
        ...
        "Medium":   "Moderate traffic. Consider carpooling or alternate routes.",
...
if __name__ == "__main__":
    print("\n" + "="*50)
    print("  CityPulse Backend Server")
    print("="*50)
